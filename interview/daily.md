...
## 脚本加载时候的`defer`与`async`是什么，有什么区别于应用场景
- defer
    - defer 属性告诉浏览器它应该继续处理页面，并在“后台”下载脚本，然后等页面处理完成后才开始执行此脚本
    - defer 属性加载的脚本总是要等待DOM解析完毕后在开始执行，且在`DOMContentLoaded`之前
    - defer 脚本加载顺序是异步的，但是执行顺序是固定的，从上到下
    - defer 仅适用于外部脚本，就是带`src`的脚本
- async
    - async 属性意味着脚本是完全独立的
    - 不会阻塞页面渲染，不会等待`DOMContentLoaded`
    - async 属性的脚本在加载完成就会立即执行，且没有上下顺序
    - 应用：不依赖应用的广告脚本，计数器等等
- 动态脚本（使用`document.createElement创建并赋值src的脚本`）
    - 默认情况下，动态脚本表现为`异步`行为
    - 不会等待其他内容，其他内容也不会等待它
    - 先加载完成的脚本先运行，即`加载优先`的顺序
- 总结
